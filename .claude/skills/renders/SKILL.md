...
2. Correr `npm run renders -- fase-N` (N = fase actual; default `fase-1`). Deja los PNG en `docs/renders/fase-N-tema-d/`, un archivo por ruta y viewport (`{ruta}--mobile-390.png`, `{ruta}--desktop-1280.png`).
   - El tema por defecto es el activo de la app (**D**). Para el alternativo: `npm run renders -- fase-N --tema=c` → `docs/renders/fase-N-tema-c/`.
   - Si el cambio a mostrar es visual, generar **ambos** y publicarlos lado a lado; si es funcional, alcanza con el tema activo.
